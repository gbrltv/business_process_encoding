...
def create_model(cases, size, window, min_count):
    ...
    """
    model = FastText(size=size, window=window, min_count=min_count, workers=-1)
...
for file in sorted_aphanumeric(os.listdir(path)):
    ...
    # generate model
    model = create_model(cases, 50, 3, 1)
    ...
    # saving
    out_df = pd.DataFrame(vectors, columns=[f'feature_{i}' for i in range(50)])
    out_df['case'] = ids
    ...
    del out_df, cases, model, ids, vectors, y
