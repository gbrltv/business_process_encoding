...
def create_model(graph, dimensions, window, min_count):
    ...
    node2vec = node2vec.fit(window=window, min_count=min_count, workers=4)
...
def average_feature_vector(cases, model):
    ...
    for case in cases:
        ...
        for i in range(len(case)-1):
            try:
                case_vector.append(model[(case[i], case[i+1])])
            except KeyError:
                pass
        if len(case_vector) == 0:
            case_vector.append(np.zeros(128))
        vectors.append(np.array(case_vector).mean(axis=0))

    return vectors

path = 'event_logs'
save_path = 'encoding_results/node2vecedges'
os.makedirs(save_path, exist_ok=True)
st = time.time()
for file in sorted_aphanumeric(os.listdir(path))[262:]:
    # reads event log
