...
def run_boostne(config, log):
    ...
    # generate model
    model = BoostNE(dimensions=config["vector_size"])
    model.fit(graph)

    # calculating feature vectors for each trace
    if config["embed_from"] == "nodes":
        vectors = trace_feature_vector_from_nodes(model.get_embedding()[:, 0:config["vector_size"]], traces, config["vector_size"], aggregation=config["aggregation"])
    elif config["embed_from"] == "edges":
        vectors = trace_feature_vector_from_edges(model.get_embedding()[:, 0:config["vector_size"]], traces, config["vector_size"], aggregation=config["aggregation"], edge_operator=config["edge_operator"])
    else:
        raise Exception("Please choose a valid graph embedding origin: {nodes, edges}")

    end_time = time.time() - start_time
    print(f"\nBoostNE took {round(end_time, 2)} seconds")
